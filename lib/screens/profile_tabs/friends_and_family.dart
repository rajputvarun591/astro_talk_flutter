import 'package:astro_talk_flutter/models/family_profile.dart';
...
import 'package:astro_talk_flutter/widgets/family_person_tile.dart';
import 'package:astro_talk_flutter/widgets/meridian_switcher.dart';
import 'package:astro_talk_flutter/widgets/my_dropdown.dart';
...
class _FriendsAndFamilyState extends State<FriendsAndFamily> {
  ...
  void didChangeDependencies() {
    ...
    Provider.of<ProfileProvider>(context, listen: true).addListener(() {
      ...
      if (pro.isDeleted != null && pro.isDeleted == true) {
        ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(const SnackBar(content: Text("Member Deleted Successfully")));
      } else if (pro.isDeleted != null && pro.isDeleted == false) {
        ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(const SnackBar(content: Text("Failed to Delete")));
      }

      if (pro.isSaved != null) {
        ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(SnackBar(content: Text(pro.isSaved!)));
  ...
  Widget build(BuildContext context) {
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Text(
                  "Wallet Balance: ₹0",
                  style: kTextStyle.copyWith(color: Colors.blue.shade900, fontSize: 16.00, fontWeight: FontWeight.w700),
                ),
                const Spacer(),
                ElevatedButton(
                  onPressed: () {},
                  child: Text(
                    "Add Money",
                    style: kTextStyle.copyWith(color: Colors.blue.shade900, fontSize: 14.00, fontWeight: FontWeight.w500),
                  ),
                  style: ElevatedButton.styleFrom(
                      primary: Colors.white,
                      shape:
                          RoundedRectangleBorder(side: BorderSide(color: Colors.blue.shade900), borderRadius: BorderRadius.circular(8.00)),
                      elevation: 0.0),
                )
              ],
            ),
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Text(
                "Add New Profile",
                style: kTextStyle.copyWith(color: Colors.black54, fontSize: 16.00, fontWeight: FontWeight.w400),
              ),
            ],
          ),
        ),
    ...
    return AnimatedSwitcher(
      ...
      transitionBuilder: (child, animation) {
        final inAnimation = Tween<Offset>(begin: const Offset(1.0, 0.0), end: const Offset(0.0, 0.0)).animate(animation);
        final outAnimation = Tween<Offset>(begin: const Offset(-1.0, 0.0), end: const Offset(0.0, 0.0)).animate(animation);
    ...
    final bool? confirmation = await showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15.00)),
            actions: [
              ElevatedButton(
  ...
  void _onSavePressed() async {
    if (_formKey.currentState!.validate()) {
      ...
      Map? place;
      final list = await NetworkService.getPlaceId(_placeOfBirthController.text);
      if(list.isNotEmpty) {
        place = list.first;
      }

      Map<String, dynamic> body = {
        "birthDetails": {"dobDay": _ddController.text, "dobMonth": _mmmController.text, "dobYear": _yyyyController.text, "tobHour": _hhController.text, "tobMin": _mmController.text, "meridiem": _meridian},
        "birthPlace": {"placeName": place != null ? place["placeName"] : null, "placeId": place != null ? place["placeId"] : null},
